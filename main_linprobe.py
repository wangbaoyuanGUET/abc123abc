...
def get_args_parser():
    parser = argparse.ArgumentParser('GCMAE linear probing for image classification', add_help=False)
...
def main(args):
    ...
    transform_val = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.6790435, 0.5052883, 0.66902906], std= [0.19158737, 0.2039779, 0.15648715])])
    dataset_train = datasets.ImageFolder(args.data_path_train, transform=transform_train)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (epoch % 20 == 0 or epoch + 1 == args.epochs or epoch == 79):
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch)
        test_stats, auc, precision, recall, f1, specificity = evaluate(data_loader_val, model, device, args.nb_classes)
