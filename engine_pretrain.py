...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (sample1, sample2,  _, index) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):

        # we use a per iteration (instead of per epoch) lr scheduler
        if data_iter_step % accum_iter == 0:
            lr_sched.adjust_learning_rate(optimizer, data_iter_step / len(data_loader) + epoch, args)

        sample1, sample2 = sample1.to(device, non_blocking=True), sample2.to(device, non_blocking=True)
        index = index.to(device, non_blocking=True)
        with torch.cuda.amp.autocast():
            # loss_mae, _, _, loss_npid, _= model(sample1, sample2, mask_ratio=args.mask_ratio, index = index, is_train=True)
            total_loss, loss_byol, loss_mae, loss_npid = model(sample1, sample2, mask_ratio=args.mask_ratio, index = index, is_train=True, npid_feature=False)
            loss = loss_mae + 0.1 * loss_npid + loss_byol
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            log_writer.add_scalar('origin_loss/train_loss_byol', loss_byol.item(), epoch_1000x)
